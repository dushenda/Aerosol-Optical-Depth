...
​	根据文献的第一种算法计算的误差不容乐观，在 <u>-0.2~0.3</u> 之间，参考黄冬师兄算的误差，这个扩大了**30**倍。说明这个算法的精度不是很高，在文献中，这个算法最后也使用了计算数据进行了比较，他给出天顶角的误差范围在 <u>0.19~0.19</u>，实际计算的比这个范围稍大些，只能说是我们这个数据不够精确或者文献的数据凑得比较好。
...
​	其实最大的不同只是 `reportFun()` 函数是不一样的，因为这个是算法的核心，主函数也是类似的，写入数据，调用函数，得到输出，最后画一下误差图。
...
​	从图上看出来，这个集散的误差范围在**-0.17与0.17**之间。对比文献，他给出天顶角的误差范围在**-0.0093与0.0093**之间，实际计算的比这个范围大了**18**倍，说明这个差距还是很大的，是有一定问题的。
...
​	算法计算的误差在**-0.17与0.17**之间。与算法三计算的差不多，文献中写的误差范围是**-0.0091与0.0091**，差了**17.5**倍左右。
...
​	图上看误差大概也是**-0.17与0.16**，文献的误差范围给的是**-0.0025与0.0027**，差距是**64**倍左右，这个差距是很大的。
...
之后对于 8 月的所有**有数据的天数**和部分九月数据进行了计算，得到线性拟合程度较好的几天。
